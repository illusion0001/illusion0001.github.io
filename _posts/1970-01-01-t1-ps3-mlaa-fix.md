...
Back in August-October 2020, I got curious and see if I could fix some of the bugs introduced with the MLAA Patch for Naughty Dog Games on the PS3.

So I asked my friend, [ZEROx](https://www.youtube.com/user/ZEROx2085/) to help out and not only did we succeeded in our goal, we mananged to improve the game overall and resolve some emulator bugs in the process. Although the crashes is indeed a real bug.
...
<em>SPU Crash; This is unavoidable.</em>
</p>

# Research

Where should I start? I dunno. But I came accross something intersting.

`0x12C41A4` Found this when I was searching for Field of View. What does it do you may ask?
...
It seems like this adjust sun position, brightness and *other* things.. there has to be more to this. Let's try Financial District, one of the subchapters in the game.

<div align="center" class="video-container">
<iframe width="560" height="315" src="https://www.youtube.com/embed/axjoXUxh7iY" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
<em>Video showing float value adjusted. Spedup by 30x</em>
</div>

[Full Video](https://youtu.be/beM6J7mVIeQ)

Welp, it didn't crash. Let's try incresing it just slightly.

<p align="center">
<img src="{% link assets/images/t1-ps3-mlaa-fix/t1-ps3-float-2.png %}">
<em>0.75. Crashed after a few minutes.</em>
</p>
...
So I then passed this off to ZEROx, he discovered the area where the values went to.

<p align="center">
<img src="{% link assets/images/t1-ps3-mlaa-fix/t1-ps3-ce-1.png %}">
</p>

Adjusting one of these values gives us fine control over the floats.

`3F 80 00 00 01 31 89 E8 01 50 E1 98`
...
With adjustments to the float, we are left with the glow seen in MLAA Path but without the performance hit.
...
<em>Seem like it is.</em>
</p>

ZEROx came to the rescue and made a list of values used throughout the game.

Columns belows contains memmory address and their values loaded in sections of the game.
...
Item:                         e190     e194   Cargo     e19c     e1a0     e1a4    e1a8    e1ac   Fog-e610
...
Patch only covers 3 problematic places: Menu, Bills town and Ending.

It compares floats as 4 bytes with certain values which are unique for those levels.

For example, sinppet of code for main menu bloom patch.
...
And if it hits one condition it applies coef, its needed for the first value or two values by using fmuls. (i.e 0.199 * 1.00 or 0.199 * 0.5 )

0.199 is the actual float value, multiplied by base which is 1.00 as its default.

150E198 must always be 0, doesn't matter if it uses positive or negative values.

Since it affects bloom glow and negative values seen in The Cargo causing a black screen.

150E1A0 must always be 0, this affects aiming reticles.
...
Looks like it test against float below at `0x12bec38` and branches to `0x95b924` when it meet its condition.

An easy patch here is to make it branch regardless of its condition check.

`0095b8f8 48 00 00 2c     b        LAB_0095b924`

Result is no crashing during the following sections.
...
ZEROx hunt down one more crash point.
...
Same patching method can be used here. Branch regardless of condition.
